...
func (g Galleries) UploadImage(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		http.Error(w, "Something went wrong(Upload Image)", http.StatusInternalServerError)
		return
	}
	...
	for _, fileHeader := range fileHeaders {
		...
		if err != nil {
			http.Error(w, "Something went wrong(inside for)", http.StatusInternalServerError)
			return
		}
		defer file.Close()
